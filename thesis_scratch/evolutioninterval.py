...
class Evolution_Valdez(EvolutionInterval):
    ...
    def F(self, x_0, h = None):
        ...
        xpts = list()
        x_current = x_0
        xpts.append(x_current)
        # Stepping through "time".
        for h_k in h:
            x_k = xpts[-1]
            # print("x_k x_k x_k x_k")
            # print(type(x_k))
            # print(x_k)
            ...
            ### Integrate Taylor approximation
    ...
    def refine(self, x_k, h_k, Y_0):
        ...
        def condition(x_k, h_k, Y_i):
            return x_k + h_k * self.f(Y_i) in Y_i

        while not condition(x_k, h_k, Y_i):
            Y_i = x_k + h_k * self.f(Y_i)
...
def main():
    ...
    print(ev.F(tupleToIntervalVector(default_start) ))
