...
def random_start(scale = 5, offset = 2):
    return [ random.random() * scale - offset for i in range(4) ]

def randscale(scale = 5):
    return random.random() * scale - scale * 0.5
...
def main():
    ...
    expmt.setParams(hyperplane = HyperPlane(randscale(), randscale(), randscale(), randscale(), randscale()), 
                    start_pts = [[9.1, 4.1, 3.2, 4.5]] + [random_start() for i in range(6) ]  )

    print("============")
    print(expmt)

    plt = expmt.run(T = 30, stepCnt = 100000)
    plt.show()
